...
CREATE TABLE IF NOT EXISTS `db_captain`.`Game` (
  ...
  `finished` VARCHAR(10) NULL DEFAULT 'no',
  PRIMARY KEY (`idGame`, `idPlayer`, `idPlanet`),
  INDEX `fk_Game_Players1_idx` (`idPlayer` ASC),
  INDEX `fk_Game_Planets1_idx` (`idPlanet` ASC),
  CONSTRAINT `fk_Game_Players1`
    FOREIGN KEY (`idPlayer`)
    REFERENCES `db_captain`.`Players` (`idPlayer`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Game_Planets1`
...
-- -----------------------------------------------------
-- Table `db_captain`.`Details`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `db_captain`.`Details` ;

CREATE TABLE IF NOT EXISTS `db_captain`.`Details` (
  `idDetails` INT NOT NULL AUTO_INCREMENT,
  `idGame` INT NOT NULL,
  `level` INT NULL,
  `game_start` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `game_end` DATETIME ON UPDATE CURRENT_TIMESTAMP,
  `attempts` INT NULL,
  `idLevel` INT NULL,
  `idQuestion` INT NULL,
  PRIMARY KEY (`idDetails`, `idGame`),
  CONSTRAINT `fk_Details_Game1`
    FOREIGN KEY (`idDetails`)
    REFERENCES `db_captain`.`Game` (`idGame`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
