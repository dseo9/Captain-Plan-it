<?php include 'statics/header.php';?>
<?php  ?>
  <div class="row mainbody ">
    <div class="title_header"><h5>Foodiverse</h5></div>
    <br/><br/>
    <div class="row subbody container-fluid">
      <div class="row" id="planets">
        <div class="col-xs-5 col-sm-5 col-md-4 col-md-offset-1 col-lg-4 col-lg-offset-1">
          <a href="fruit.php?p_name=Fruit Fortress"><img src="images\fruit_fortress.png" alt="Fruit_Fortress" class="img-responsive center-block"/></a>
        </div>
        <div class="col-xs-5 col-xs-offset-2 col-sm-5 col-sm-offset-2 col-md-4 col-md-offset-2 col-lg-4 col-lg-offset-2">
          <a href="pastry.php?p_name=Pastry Palace"><img src="images\pastry_palace.png" alt="Pastry_Palace" class="img-responsive center-block"/></a>
        </div>
      </div>

      <div class="row" id="planets"></div>

      <div class="row" id="planets">
        <div class="col-xs-5 col-sm-5 col-md-4 col-md-offset-1 col-lg-4 col-lg-offset-1">
          <a href="dairy.php?p_name=Dairy Dome"><img src="images\dairy_dome.png" alt="Fruit_Fortress" class="img-responsive center-block"/></a>
        </div>
        <div class="col-xs-5 col-xs-offset-2 col-sm-5 col-sm-offset-2 col-md-4 col-md-offset-2 col-lg-4 col-lg-offset-2">
          <a href="meat.php?p_name=Meat Mansion"><img src="images\meat_mansion.png" alt="Pastry_Palace" class="img-responsive center-block"/></a>
        </div>
      </div>

    </div><!-- ROW SUBBODY END-->
  </div><!-- ROW MAINBODY END-->

<?php include 'statics/footer.php' ;?>
